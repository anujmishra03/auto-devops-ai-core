...
jobs:
  build:
    ...
    steps:
      ...
      # Only run Node pipeline if package.json exists
      - name: Setup Node.js
      ...
      # No Node project? Just pass CI.
      - name: No Build Needed
        if: steps.detect.outputs.project_type == 'unknown'
        run: echo "No package.json found → Passing CI automatically."
`;
}
...
export async function POST(req) {
  try {
    ...
    console.log("➡️ Using branch:", branch);
    ...
    try {
      ...
      if (
        ...
        console.log("ℹ️ Branch already exists, continuing:", branch);
      } else {
    ...
    // 4) commit CI file
    console.log(
      "➡️ Committing CI workflow to .github/workflows/ci.yml..."
    );
    try {
      await axios.put(
      ...
      if (axios.isAxiosError(err) && err.response) {
        console.log("❌ Error committing CI:", {
          status: err.response.status,
          data: err.response.data,
        });
      }
    ...
    // 5) create PR
    console.log("➡️ Creating PR...");
    let pr;
    try {
      pr = await axios.post(
        `https://api.github.com/repos/${repoName}/pulls`,
        {
          title: `🤖 AutoDevOpsAI — ${next_action}`,
    ...
    if (axios.isAxiosError(err) && err.response) {
      console.error("❌ PR create error (GitHub):", {
        status: err.response.status,
        data: err.response.data,
      });
    ...
    console.error("❌ PR create error (unexpected):", err);
    return Response.json(
      {
        ok: false,
        error: "Failed to create PR",
